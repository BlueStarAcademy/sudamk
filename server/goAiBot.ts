...
/**
 * 히든 돌 공개 연출 종료 직후: KataServer 세션 캐시를 현재 판으로 맞춘다.
 * 주의: `/move` 응답 후보를 저장해 다음 AI 틱에 재사용하지 않는다.
 * (사람 차례에 프라임하면 side-to-move 불일치 후보가 남고, 이후 AI가 엉터리 합법수를 둘 수 있음)
 */
export async function primeKataServerBoardAfterHiddenReveal(game: types.LiveGameSession): Promise<void> {
    ...
    // 이전 버전의 stale 프라임 후보는 절대 쓰지 않음
    (game as any).pendingKataCandidatesAfterHiddenReveal = undefined;

    const hiddenRevealTag = bumpKataHiddenRevealCacheTag(game);
    const aiToMove = game.currentPlayer === aiPlayerEnum;
    // AI 착수 직전 재개일 때만 하드 리싱크(인코딩 재시도 강화). 사람 차례 프라임에서는 켜지 않음.
    if (aiToMove) {
        (game as any).kataHardResyncAfterHiddenReveal = true;
    }

    // 사람 차례면 캐시 태그 bump만 하고 /move는 호출하지 않음(AI color로 질의하면 STM 불일치).
    if (!aiToMove) {
        if (process.env.NODE_ENV === 'development') {
            console.log(
                `[primeKataServerBoardAfterHiddenReveal] cache-tag only (human to move) game=${game.id} tag=hr-post-${hiddenRevealTag}`,
            );
        }
        return;
    }
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.log(
                `[primeKataServerBoardAfterHiddenReveal] warmed Kata cache game=${game.id} kataMoves=${moveHistory.length} tag=hr-post-${hiddenRevealTag}`,
            );
        }
    } catch (e: any) {
        console.warn(
...
/**
 * 전략바둑 대표펫 힌트: 인간 차례 색으로 Kata 1수(합법 수·히든 AI대국 마스킹 규칙은 AI 착수와 동일).
 */
export async function computeStrategicPetKataHintMove(
    game: types.LiveGameSession,
    ...
    for (const cand of kataCandidates) {
        ...
        const trial = processMove(
            game.boardState,
            { ...cand, player: humanPlayerEnum },
            game.koInfo,
            game.moveHistory.length,
        );
        if (trial.isValid) return cand;
...
function buildKataMoveHistory(
    ...
    if (
        ...
        // 히든 PVE: Kata 입력은 AI가 보는 판과 동일해야 함(전체 판이면 미공개 흑이 들어가 국면이 틀어짐)
        return appendPassesUntilSideToMove(
            encodeBoardStateAsKataSetupMovesFromEmpty(replayTargetBoard),
            game.currentPlayer,
        );
    }
...
/**
 * Kata 전용 경로가 모두 실패했을 때의 최후 수단.
 * 스캔 순서 첫 합법수(완전 엉터리) 대신 가벼운 보드 점수로 고른다 — 정상 대국은 Kata 경로를 써야 한다.
 */
...
function clearKataHiddenRevealResyncFlags(game: types.LiveGameSession): void {
...
function buildKataMoveHistoryForExclusivePick(
    ...
    return buildKataMoveHistoryBoardOnlyFromAiVisibleBoard(game, aiPlayerEnum);
}
...
function buildKataMoveHistoryBoardOnlyFromAiVisibleBoard(
    ...
    const bs = game.settings?.boardSize ?? game.boardState?.length ?? 19;
    const board = getBoardStateForAi(game, aiPlayerEnum);
    if (
        board?.length === bs &&
        board.every((row: unknown[]) => Array.isArray(row) && row.length === bs)
    ) {
        return appendPassesUntilSideToMove(
...
async function pickKataMoveExclusiveWithBoardResync(params: {
    ...
    isHiddenMode: boolean;
    tagSuffix: string;
}): Promise<KataExclusivePickResult> {
    const {
        game,
        aiPlayerEnum,
        opponentPlayerEnum,
    ...
    for (let resync = 0; resync < maxResync; resync++) {
        ...
        if (kataCandidates.length > 0) {
            preferHistoryEncoding = true;
            (game as any).kataPveKataMovesFromBoardStateOnly = false;
        }
...
export async function makeGoAiBotMove(
    ...
    // 베이스·착색 등 본대국 전 단계에서 Kata/일반 착수를 실행하면 수순·베이스·히든 메타가 꼬일 수 있음
    const strategicGoMainBoardAllowedStatuses = new Set([
        'playing',
        'hidden_placing',
        'scanning',
        'scanning_animating',
    ...
    const gsGuard = String(game.gameStatus);
    if (!strategicGoMainBoardAllowedStatuses.has(gsGuard)) {
        if (process.env.NODE_ENV === 'development') {
            const mh = (game.moveHistory || []).filter((m) => m && typeof m.x === 'number' && typeof m.y === 'number' && m.x >= 0 && m.y >= 0).length;
            if (mh > 0) {
                console.warn(
                    `[makeGoAiBotMove] Skipping main-board AI: gameStatus=${gsGuard} playableMoves=${mh} game=${game.id}`,
                );
            }
        }
        return;
    ...
    if (sessionUsesCastleGo(game) && game.gameStatus === 'playing') {
    ...
    if (game.aiHiddenItemAnimationEndTime != null && now >= game.aiHiddenItemAnimationEndTime) {
        ...
        const usedAfterAnim = Math.max(0, Number((game as any).aiHiddenItemsUsedCount ?? 0));
        const recordedAfterAnim = countRecordedAiHiddenMoves(game, aiPlayerEnum);
        const owesHiddenStoneFromAnimation = recordedAfterAnim < usedAfterAnim;
        (game as any).pendingAiHiddenPlacement = Boolean(
            isHiddenMode &&
                (wasAiThinkingHiddenReveal || thinkPlacementDue) &&
                (aiHiddenLeft > 0 || owesHiddenStoneFromAnimation)
        );
    ...
    /** 싱글/탑: 예정 횟수를 이미 모두 "연출 시작(+1)" 한 뒤면 추가 연출을 막는다 */
    const singlePlayerPlannedHiddenUsesCap =
        game.isSinglePlayer && isHiddenMode && !disableAiHiddenItemsByStageSetting
            ? Math.max(
                  configuredSinglePlayerHiddenTurns.length,
                  normalizeAiHiddenTurns((game as any).aiHiddenItemTurns).length,
              )
            : 0;
    ...
    if (
        ...
        !atOrOverSinglePlayerHiddenSchedule &&
        !atOrOverTowerHiddenSchedule
    ) {
        const appliesToAi =
            game.isSinglePlayer ||
            (game as any).gameCategory === 'tower' ||
        ...
        if (aiHiddenLeft > 0 && appliesToAi) {
            ...
            // 연출 구간에는 turnDeadline이 흘러 AI 시계/타임아웃·스피드 추정이 꼬이지 않도록 마감을 연출 길이만큼 연장한다.
            if (game.turnDeadline != null && Number.isFinite(game.turnDeadline)) {
                game.turnDeadline += AI_HIDDEN_ITEM_THINKING_DURATION_MS;
            }
    ...
    if (sessionUsesChessGo(game) && !game.chessPieceMovedThisTurn) {
    ...
    if (game.isSinglePlayer && shouldApplyHiddenOnThisMove && configuredAiHiddenPlacements.length > 0) {
        // 연출 직전에 used가 +1 되므로 N번째 히든 착수는 recorded(0-based)로 고른다.
        const placementByOrder = configuredAiHiddenPlacements[recordedAiHiddenMoves];
    ...
    if (!selectedMove) {
        ...
        // 히든 공개 프라임 후보를 절대 재사용하지 않음 — 항상 현재 국면에서 새 Kata 질의
        (game as any).pendingKataCandidatesAfterHiddenReveal = undefined;
        ...
        if (!selectedMove && !pickedFromKata) {
            if (!isKataServerAvailable()) {
                ...
                // 공개 직후 1회만 강화 재시도 — 실패해도 hardResync를 영구 고정하지 않음
                ...
                if (retryPick.picked) {
                    ...
                    if (fallbackMove) {
                        ...
                        console.warn(
                            `[makeGoAiBotMove] Kata-only exhausted after retries; using scored legal fallback (${fallbackMove.x},${fallbackMove.y}), game=${game.id}`,
                        );
                    } else if (pairHasFixedScoringTurnLimit && pairCurrentSeat) {
                        const totalTurns = getProgressTurnCount(game);
                        console.warn(
    ...
    if (
        ...
        if (isKataServerAvailable()) {
            const noPassPick = await pickKataMoveExclusiveWithBoardResync({
                game,
                aiPlayerEnum,
                opponentPlayerEnum,
    ...
    if (isHiddenMode && shouldMaskUserHiddenFromAi(game)) {
        ...
        if (isTargetHiddenOpponentStone) {
            ...
            if (!pickedReveal) {
                ...
                console.warn(
                    `[makeGoAiBotMove] Kata re-query after hidden reveal failed; using scored legal fallback (${fallbackRevealMove.x},${fallbackRevealMove.y}), game=${game.id}`,
                );
    ...
    if (!result.isValid) {
        ...
        if (isStrictForcedResponse) {
            console.warn(
                ...
                await summaryService.endGame(game, opponentPlayerEnum, 'resign');
                return;
        } else if (isKataServerAvailable()) {
            const emergencyPick = await pickKataMoveExclusiveWithBoardResync({
                game,
                aiPlayerEnum,
                opponentPlayerEnum,
                kataLevel: resolvedKataLevel,
            ...
            if (!result.isValid) {
                ...
                if (fallbackMove) {
                    ...
                    if (retry.isValid) {
                        console.warn(
                            `[makeGoAiBotMove] Kata-only emergency invalid; using scored legal fallback (${fallbackMove.x},${fallbackMove.y}), game=${game.id}`,
                        );
            ...
            if (!result.isValid) {
                throw new Error(
                    `[makeGoAiBotMove] 선택 수가 규칙상 무효입니다 (${selectedMove.x},${selectedMove.y}). game=${game.id}`,
                );
            }
        }
    }

    const usedHiddenCounterForMark = Math.max(0, Number((game as any).aiHiddenItemsUsedCount ?? 0));
    ...
    if (treatAsAiHiddenStone) {
        ...
        (game as any).aiHiddenItemThinkDueMoveCount = undefined;
        pendingAiHiddenKataMoveByGameId.delete(hiddenKataCacheKey(game.id));
    } else if (shouldApplyHiddenOnThisMove) {
        // stale pending: 일반돌 착수로만 반영하고 사용 카운터·플래그를 수순에 맞게 되돌림
        (game as any).aiHiddenItemsUsedCount = countRecordedAiHiddenMoves(game, aiPlayerEnum);
        (game as any).pendingAiHiddenPlacement = false;
        (game as any).aiHiddenPlacementArmed = false;
        (game as any).aiHiddenItemThinkPlacementDue = false;
        (game as any).aiHiddenItemThinkDueMoveCount = undefined;
    ...
    if (!isItemModeHere && shouldApplyBlackTurnLimit && blackTurnLimit !== undefined && aiPlayerEnum === types.Player.White && game.gameStatus === 'playing') {
        ...
        if (blackMoves >= blackTurnLimit) {
            ...
            if (game.stageId) {
                ...
                const autoScoringTurns = resolveSinglePlayerAutoScoringTurnCap(game.settings as any, stage);
