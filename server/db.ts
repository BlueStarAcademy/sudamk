...
export const getAllUsers = async (options?: { includeEquipment?: boolean; includeInventory?: boolean; skipCache?: boolean }): Promise<User[]> => {
    // equipment/inventory가 필요 없는 경우에만 캐시 사용 (skipCache면 도전의 탑 랭킹 등 최신 반영 필요 시 캐시 미사용)
    const needsEquipment = options?.includeEquipment ?? false;
    const needsInventory = options?.includeInventory ?? false;
    const skipCache = options?.skipCache ?? false;
    const canUseCache = !needsEquipment && !needsInventory && !skipCache;
