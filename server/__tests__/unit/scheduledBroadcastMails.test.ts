...
describe('scheduledBroadcastMails', () => {
    ...
    it('builds weekday lunch mail with 30 action points', () => {
    ...
    it('builds weekday evening mail with material box II', () => {
    ...
    it('builds weekend evening mail with material box IV and 10 diamonds', () => {
        const spec = buildScheduledBroadcastMailSpec('evening', true);
        expect(spec.message).toContain('주말은 너무 행복해요');
        expect(spec.attachments.diamonds).toBe(10);
        expect(spec.attachments.items).toEqual([{ itemId: '재료상자4', quantity: 1 }]);
    });
