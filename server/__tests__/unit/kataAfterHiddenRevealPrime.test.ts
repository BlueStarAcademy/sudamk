...
const generateKataMock = vi.fn(async () => ({
    candidates: [
        { x: 4, y: 4 },
        { x: 2, y: 3 },
    ],
...
vi.mock('../../kataServerService.js', () => ({
    ...
    generateKataServerMoveCandidateDetails: (...args: unknown[]) => generateKataMock(...args),
}));
...
describe('primeKataServerBoardAfterHiddenReveal', () => {
    ...
    it('warms Kata when AI to move but does not store reusable candidates', async () => {
        ...
        expect(generateKataMock).toHaveBeenCalledTimes(1);
        expect((game as any).kataHardResyncAfterHiddenReveal).toBe(true);
        expect((game as any).pendingKataCandidatesAfterHiddenReveal).toBeUndefined();
    });

    it('skips Kata /move when human to move (avoids stale AI candidates)', async () => {
        const { primeKataServerBoardAfterHiddenReveal } = await import('../../goAiBot.js');
        ...
        (game as any).pendingKataCandidatesAfterHiddenReveal = [{ x: 1, y: 1 }];
        (game as any).kataHardResyncAfterHiddenReveal = true;

        await primeKataServerBoardAfterHiddenReveal(game);

        expect(generateKataMock).not.toHaveBeenCalled();
        expect((game as any).pendingKataCandidatesAfterHiddenReveal).toBeUndefined();
        expect((game as any).kataHardResyncAfterHiddenReveal).toBe(true);
    });
});
