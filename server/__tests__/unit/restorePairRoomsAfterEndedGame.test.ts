...
import { restorePairRoomsAfterEndedGame, leavePairWaitingRoomIfPresent } from '../../actions/socialActions.js';
...
describe('leavePairWaitingRoomIfPresent', () => {
    it('keeps an in-game friendly room shell instead of dissolving it', () => {
        const volatileState: any = {
            userStatuses: {
                'owner-1': { status: 'in-game', gameId: 'g1' },
                'guest-1': { status: 'in-game', gameId: 'g1' },
            ...
            pairRooms: {
                'pair-room-friendly-1': {
                    id: 'pair-room-friendly-1',
                    code: '1',
                    mode: 'pvp',
                    pairMode: 'pvp',
                    roomKind: 'duo_match',
        ...
        expect(leavePairWaitingRoomIfPresent(volatileState, 'owner-1')).toBe(false);
        expect(volatileState.pairRooms['pair-room-friendly-1']).toBeTruthy();
        expect(volatileState.pairRooms['pair-room-friendly-1'].ownerId).toBe('owner-1');
        expect(volatileState.pairRooms['pair-room-friendly-1'].partnerId).toBe('guest-1');
    });
});
