...
/**
 * 페어 일반전 매칭 전용 숨은 MMR (`matchScore`).
 * 페어 랭킹점수·티어와 분리 — 페어 일반전 결과로만 갱신, 최초 시드는 pair rankingScore.
 */
export const PAIR_NORMAL_STAT_KEY = 'pairNormal';

export type PairNormalStatBlock = {
    matchScore: number;
};

/** 페어 일반전 전적(사람+펫 일반 매칭 큐 PvP만) */
export const PAIR_NORMAL_MATCH_RECORD_KEY = 'pairNormalMatchRecord';
