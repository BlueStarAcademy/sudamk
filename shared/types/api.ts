...
export interface VolatileState {
    ...
    pairRankedPetProposals?: Record<
        string,
        {
            ...
            createdAt: number;
            acceptDeadlineAt: number;
            /** 기본(미지정)=페어 펫 랭크; 2인 페어 인간 팀 랭크 매칭 */
...
export interface PairRoomState {
    ...
    /** 전략·놀이 경기장 2인 페어: 방장이 랭킹전 모드 제안 후 파트너가 동의하기 전 */
...
export type ServerAction =
    ...
    | { type: 'PAIR_START_AI_MATCH'; payload?: {
          mode?: GameMode;
          settings?: GameSettings;
          roomId?: string;
          /** 방 없이 즉시 시작 시 경기 종류 */
    ...
    | { type: 'CLAIM_STRATEGIC_PET_HINT_BONUS', payload: { gameId: string; x: number; y: number; expectedMoveHistoryLength?: number; missileLand?: boolean } }
    ...
    /** 프로필 관리자 테스트: 다이아 패키지 ON은 BUY_CASH_PACKAGE와 동일, OFF는 해당 티어 활성 시만 해제 */
    ...
    | { type: 'SINGLE_PLAYER_SYNC_PENDING_STAGE', payload: { gameId: string; regenerateBoard?: boolean } }
