 ...
 * (클라이언트·서버 공통 — `utils/championshipLiveScores` 등에서 사용)
 */
 ...
 */
export function calculateScoreManually(
    ...
    if (!silent) console.log(`[ScoringService] Territory: Black=${territory.black}, White=${territory.white}`);
    ...
    if (!silent) console.log(`[ScoringService] Captures: Black=${blackCaptures}, White=${whiteCaptures}`);
    ...
    if (!silent) console.log(`[ScoringService] Final scores: Black=${blackScore}, White=${whiteScore} (komi=${komi})`);
    ...
    if (!silent) console.log(`[ScoringService] Manual score calculation completed for game ${session.id}`);
    return result;
}
