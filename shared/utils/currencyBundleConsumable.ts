/**
 * 골드/다이아 꾸러미 소모품 이름을 서버 `currencyBundles` 키 형태로 통일 (표기·공백·로마숫자·아라비아 숫자 호환).
 */
export function resolveCurrencyBundleConsumableKey(name: string): string | null {
    ...
    const romanTier = n.match(/^(골드|다이아)\s+꾸러미\s+(I|II|III|IV)$/);
    if (romanTier) {
        return `${romanTier[1]} 꾸러미 ${romanTier[2]}`;
    }
