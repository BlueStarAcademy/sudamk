...
## 백엔드/프론트 분리와 크래시
**백엔드와 프론트를 분리한 것이 크래시 원인은 아닙니다.**  
분리하면 API 서비스는 정적 파일·Vite를 서빙하지 않아 메모리 부담이 줄어들 수 있습니다.  
로그에 에러 없이 재시작되는 경우는 보통 **OOM(메모리 초과)** 또는 **헬스체크 타임아웃**으로 플랫폼이 컨테이너를 재시작하는 경우입니다.
...
## 로그에서 재시작만 보일 때
- **PID가 바뀌고** 그 직전에 `npm notice` 등이 보이면, **프로세스가 죽은 뒤 컨테이너가 다시 올라가서** `npm run start-server`가 새로 실행된 것입니다.
- 이때 애플리케이션은 `[CRASH_REASON]`이나 `SIGTERM` 로그를 남길 기회가 없을 수 있음 → **OOM(SIGKILL)** 또는 **플랫폼이 헬스 실패로 재시작**한 경우가 많습니다.
- **Railway Dashboard → Metrics → Memory**에서 크래시 직전 메모리 곡선을 확인하세요.
