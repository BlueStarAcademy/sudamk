...
### 🌐 같은 네트워크(LAN)에서 PVP 테스트하기

같은 Wi‑Fi/랜에 있는 다른 컴퓨터(스마트폰 포함)에서 접속해 **멀티플레이/PVP**를 테스트할 수 있습니다.

#### 1) 호스트(서버를 실행하는 PC)에서 실행

- **(권장) 방화벽 포트 열기**: 관리자 권한 PowerShell에서 아래 스크립트를 실행합니다.
...
> 이 프로젝트는 `vite --host`로 실행되므로, Vite 개발 서버(5173)가 자동으로 모든 네트워크 인터페이스에 바인딩됩니다.
...
> 고정 IP가 필요하면 `IP설정안내.txt`, `set-ip.bat`, `set-static-ip.ps1`를 참고하세요.

#### 3) 다른 PC에서 접속

- 브라우저에서 아래로 접속:
  - 프론트(개발 서버): `http://<호스트IP>:5173`
  - (선택) API/헬스체크: `http://<호스트IP>:4000/api/health`

#### 4) PVP 테스트 팁

- **서로 다른 계정**으로 로그인하거나, 한쪽은 **시크릿/인프라이빗 창**을 사용하세요.
- PVP/실시간 기능은 **WebSocket(`/ws`)** 를 사용합니다. 연결이 안 되면 방화벽/공유기(AP) 설정에서 5173/4000 포트를 확인하세요.

#### 자주 막히는 원인

- 같은 네트워크가 아님(게스트 Wi‑Fi/다른 대역)
- Windows 방화벽이 포트를 차단
- 공유기에서 “클라이언트 격리(AP Isolation)” 옵션이 켜짐
