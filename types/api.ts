...
export interface PairRoomChatLine {
    ...
    text: string;
    timestamp: number;
    /** 방 안 모두(상대 팀 포함) vs 같은 팀만 */
    scope: 'room' | 'team';
    /** scope가 team일 때 해당 팀만 수신; room이면 발신자 팀(표시용) */
    teamId: 'teamA' | 'teamB';
}
...
export interface PairRoomState {
    ...
    /** PVP 방: 방장이 드래그로 배치한 유저 id (팀당 최대 2). 없으면 방장+파트너를 A팀으로 간주 */
...
export interface UserStatusInfo {
    ...
    /** 페어 경기장(#/pair) 화면에 있음 — 전체 목록 초대 조건에 사용 */
    inPairLobby?: boolean;
}
...
export type ServerAction =
    ...
    /** 클라·서버 수련 슬롯 불일치 복구: 서버 진실값 반환 + 진행 중 고아 세션만 DB에서 정리 */
