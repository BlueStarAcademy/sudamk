...
export async function executeUseConditionPotionAction(
    ...
    try {
        ...
        if (!res.ok) {
            let errorMessage = '회복제 사용에 실패했습니다.';
            try {
            ...
            deps.showError(typeof errorMessage === 'string' ? errorMessage : '회복제 사용에 실패했습니다.');
        ...
        deps.applyUserUpdate(revertUser, 'USE_CONDITION_POTION-revert');
        const message = err instanceof Error ? err.message : '회복제 사용 중 오류가 발생했습니다.';
