...
const DEMO_SCORING_HOLD_MS = 1100;
const MISSILE_DEMO_STEP_MS = 850;
const MISSILE_FLIGHT_MS = 600;
const MISSILE_CAPTURE_MS = 700;
/** 튜토리얼용 가속 틱 — 실제 1초보다 짧게 10→0 연출 */
const SPEED_TICK_MS = 450;
const SPEED_SCORE_HOLD_MS = 1200;
...
const PveInteractiveTutorialModal: React.FC<Props> = ({ tutorialId, onComplete, onClose }) => {
    ...
    useEffect(() => {
        if (isMissile || isSpeed || phase !== 'demo' || !demoPlaying || !lesson) return;
        clearTimer();
        ...
        return () => clearTimer();
    }, [isMissile, isSpeed, phase, demoPlaying, demoIndex, demoEpoch, lesson, clearTimer]);
    ...
    const demoStones: PveTutorialStone[] = useMemo(() => {
        ...
        return stones;
    }, [lesson, demoIndex, missileDemo, missileStep, applyMissileBoard]);
    ...
    const practiceCompleteStones: PveTutorialStone[] = useMemo(() => {
        ...
        return stones;
    }, [lesson, practiceSequence, missileDemo, applyMissileBoard]);
    ...
    const practiceStones: PveTutorialStone[] = useMemo(() => {
        ...
        return stones;
    }, [lesson, practiceBase, practiceIndex, practiceSequence, missileDemo, missileStep, applyMissileBoard]);
    ...
    const visibleTerritory: PveTutorialStone[] | null = (() => {
        if (isMissile || isSpeed) return null;
    ...
    const showCaptureFlash =
        isMissile &&
        (missileStep === 'capture' || (phase === 'done' && (missileStep === 'done' || missileStep === 'capture')));
                ...
                <>
                    ...
                    {skipPractice ? (
                        ...
                        <Button
                            ...
                            cooldownMs={0}
                        >
                            {t(isMissile ? 'pveBrief.tutorialMissilePractice' : 'pveBrief.tutorialPractice')}
                        </Button>
                    )}
                </>
            ) : phase === 'practice' ? (
                        ...
                        {showCaptureFlash ? (
                            ...
                            >
                                <span className="rounded-full border border-rose-300/50 bg-black/75 px-3 py-1 text-[11px] font-black tracking-wide text-rose-100 shadow-lg sm:text-xs">
                                    {t('pveBrief.tutorialMissileCaptureFlash')}
                                </span>
                            </div>
                        ) : null}
                    </div>
