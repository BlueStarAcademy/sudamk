...
const Icon: React.FC<{ kind: NavKind }> = ({ kind }) => {
            ...
            {GAME_RECORD_REPLAY_NAV_ORDER.map((kind) => {
                ...
                return (
                    <button
                        ...
                        className="group flex h-11 w-11 items-center justify-center rounded-xl border border-amber-400/35 bg-gradient-to-b from-zinc-800/95 to-zinc-950 text-amber-100 shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_4px_12px_rgba(0,0,0,0.35)] transition hover:border-amber-300/55 hover:from-zinc-700/95 hover:to-zinc-900 hover:text-amber-50 disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:border-amber-400/35 disabled:hover:from-zinc-800/95 disabled:hover:to-zinc-950 sm:h-12 sm:w-12"
                    >
                        <Icon kind={kind} />
                    </button>
                );
