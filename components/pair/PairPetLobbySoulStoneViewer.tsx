...
const PairPetLobbySoulStoneViewer: React.FC<PairPetLobbySoulStoneViewerProps> = ({
    ...
    return (
        ...
        >
            ...
            <div className="relative z-[1] min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-2 pb-1.5 pt-2 [-webkit-overflow-scrolling:touch] [scrollbar-width:thin] sm:px-4 sm:pb-2 sm:pt-4">
            <div className="relative flex flex-col gap-1.5 sm:gap-2.5">
                <div className="flex flex-row items-start gap-2 sm:gap-4">
                    ...
                    <div className="flex min-w-0 flex-1 flex-col gap-1 sm:gap-2">
                        <div className="flex items-start justify-between gap-1.5 sm:gap-2">
                            <h3 className="min-w-0 flex-1 text-left text-[0.8125rem] font-black leading-snug tracking-tight text-violet-50 drop-shadow-[0_2px_14px_rgba(88,28,135,0.5)] min-[380px]:text-[0.875rem] sm:text-lg md:text-xl">
                                {item.name}
                            </h3>
                            <span
                                className="shrink-0 rounded-md border border-amber-400/35 bg-gradient-to-b from-amber-900/40 to-black/50 px-1.5 py-0.5 text-center text-[0.58rem] font-extrabold leading-none text-amber-100 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] ring-1 ring-inset ring-amber-300/15 sm:rounded-lg sm:px-2.5 sm:py-1.5 sm:text-xs"
                                title="보유 개수"
                            >
                                보유
                                <span className="mt-px block tabular-nums text-[0.68rem] text-amber-50 sm:mt-0.5 sm:text-sm">×{qty.toLocaleString()}</span>
                            </span>
                        </div>
                        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                            <span
                                className={`inline-flex items-center gap-1 rounded-md border border-white/12 bg-black/40 px-1.5 py-0.5 text-[0.65rem] font-bold tabular-nums ring-1 ring-inset ring-white/[0.06] sm:gap-1.5 sm:rounded-lg sm:px-2.5 sm:py-1 sm:text-xs ${gradeMeta.color}`}
                            >
                                <span className="text-[0.58rem] font-semibold uppercase tracking-[0.12em] text-slate-500 sm:text-[0.65rem] sm:tracking-[0.14em]">등급</span>
                                <span className="text-[0.7rem] sm:text-sm">{gradeMeta.name}</span>
                            </span>
                        </div>

                        {item.description ? (
                            <p className="rounded-lg border border-white/[0.07] bg-black/35 px-2 py-1.5 text-[0.65rem] leading-snug text-slate-300/95 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] ring-1 ring-inset ring-violet-500/10 sm:rounded-xl sm:px-3 sm:py-2 sm:text-xs sm:leading-relaxed md:text-[0.8125rem]">
                                {item.description}
                            </p>
                        ) : null}
                    </div>
                ...
                <div className="w-full rounded-lg border border-cyan-500/18 bg-gradient-to-r from-slate-950/90 via-[#0c0a12] to-slate-950/90 px-2 py-1.5 ring-1 ring-inset ring-cyan-500/10 sm:rounded-xl sm:px-3.5 sm:py-2.5">
                    <p className="mb-1 text-[0.65rem] font-semibold text-cyan-200/80 sm:mb-1.5 sm:text-xs">사용처</p>
                    <p className="break-words font-sans text-[0.62rem] font-medium leading-snug text-slate-300/95 sm:text-xs">
                ...
                <div className="w-full rounded-lg border border-amber-500/20 bg-gradient-to-r from-[#1a1510]/92 via-[#0f0c0a] to-[#0a0908] px-2 py-1.5 ring-1 ring-inset ring-amber-500/12 sm:rounded-xl sm:px-3.5 sm:py-2.5">
                    <p className="mb-1 text-[0.65rem] font-semibold text-amber-200/85 sm:mb-1.5 sm:text-xs">획득처</p>
                    <div className="flex flex-col gap-1 font-sans text-[0.62rem] font-medium leading-snug text-slate-300/95 sm:gap-1.5 sm:text-xs">
