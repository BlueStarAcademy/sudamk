...
export interface PairPetHomeEmptyDetailFrameProps {
    ...
    /** {@link resolvePetInfoViewerEmbedLayout} — 펫 정보 뷰어와 동일 틀 */
 ...
 */
const PairPetHomeEmptyDetailFrame: React.FC<PairPetHomeEmptyDetailFrameProps> = ({
    variant,
    ...
    const rowPad = homePack ? PET_PANEL_ROW_PAD : isPanelFit ? 'p-1.5' : 'p-2 sm:p-3';
    const imgShellClass = isPanelFit
        ? homePack
            ? `relative aspect-square w-full overflow-hidden rounded-lg border border-dashed border-white/20 bg-zinc-900/70 shadow-inner ${PET_PANEL_PORTRAIT_MAX}`
    ...
    const traitRowGap = homePack ? PET_PANEL_TRAIT_GAP : isPanelFit ? 'gap-1 sm:gap-1.5' : 'gap-1.5 sm:gap-2';
    const traitBoxFuchsia = homePack
    ...
    const traitTitleFuchsia = homePack
    ...
    const traitTitleAmber = homePack
    ...
    const gridGap = homePack ? PET_PANEL_CORE_GRID : 'grid w-full min-w-0 grid-cols-3 gap-1.5 sm:gap-2';
    const cellPad = homePack ? '' : 'px-2 py-1.5 sm:px-2.5';
    const labelText = homePack ? 'whitespace-nowrap text-[12px] font-semibold leading-snug text-slate-500 antialiased' : 'text-[11px] text-slate-500 sm:text-xs';
    ...
    const heroTopGridCols = homePack ? PET_PANEL_HERO_GRID_COLS : 'grid-cols-[3fr_7fr]';
    const heroTopGridGap = homePack ? 'gap-x-1.5 gap-y-1' : 'gap-x-2 gap-y-1 sm:gap-x-2.5 sm:gap-y-1.5';

    const inner = (
        <div className={`flex w-full min-w-0 flex-col ${homePack ? PET_PANEL_BADUK_BLOCK_GAP : rootGap}`}>
            <div
            ...
            >
                ...
                >
                    ...
                    <div className="flex min-w-0 flex-col justify-center gap-1 text-left sm:gap-1.5">
                        ...
                        <div className={homePack ? 'mt-1 space-y-1' : 'mt-0.5 space-y-0.5'}>
            ...
            <div className={homePack ? PET_PANEL_BADUK_STRIP : 'flex min-w-0 flex-nowrap items-center justify-center overflow-x-auto rounded-xl border border-sky-500/30 bg-gradient-to-r from-sky-950/40 to-zinc-950/80 px-1.5 py-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] [scrollbar-width:thin]'}>
                ...
                {(['초반', '중반', '종반'] as const).map((label, idx) => (
                    <React.Fragment key={label}>
                        ...
                        <span className="inline-flex shrink-0 items-baseline gap-0.5">
                            <span className={homePack ? PET_PANEL_BADUK_PHASE_LABEL : 'text-xs text-slate-500'}>{label}</span>
                            <span className={`font-mono ${homePack ? `${PET_PANEL_BADUK_PHASE_NUM} text-sky-100/35` : 'text-sky-100/35'}`}>—</span>
                        </span>
                    </React.Fragment>
                ))}
            </div>
            ...
            <div className={gridGap}>
                {CORE_STAT_RADAR_ORDER.map((stat) => {
                    ...
                    return (
                        <div
                            key={stat}
                            className={`flex min-w-0 flex-row items-center justify-between ${homePack ? PET_PANEL_CORE_CELL : `rounded-md border border-white/10 bg-black/30 ${cellPad}`} ${homePack ? '' : 'gap-2'}`}
                        >
