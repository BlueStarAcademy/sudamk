        ...
        const handleCallback = async () => {
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(errorData.message || t('errors.googleLoginFailed'));
                }
                ...
                setError(err.message || t('callback.googleProcessError'));
                setIsLoading(false);
            }
        };
        ...
        return (
            <div className={`flex w-full min-w-0 min-h-[280px] flex-col items-center justify-center py-8 ${isNativeMobile ? 'px-3 pb-[max(1rem,env(safe-area-inset-bottom,0px))]' : ''}`}>
                <div className="flex flex-col items-center text-center">
                    ...
                    <p className="text-lg text-gray-400">{t('callback.googleProcessing')}</p>
                </div>
            </div>
        );
    }
