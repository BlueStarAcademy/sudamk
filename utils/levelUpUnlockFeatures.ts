...
export function getLevelUpFeatureUnlockLines(
    ...
    user: { userLevel?: number },
): { strategy: string[]; combined: string[] } {
    const curU = Math.max(1, Math.floor(Number(user.userLevel ?? 1) || 1));
    const strat = branchSnap(payload, 'strategy', curU);

    const strategy: string[] = [];
    const combined: string[] = [];

    if (crossedLevel(strat.from, strat.to, QUEST_MIN_STRATEGY_LEVEL)) {
        pushUnique(strategy, `퀵 메뉴 · 퀘스트 이용 (유저 Lv.${QUEST_MIN_STRATEGY_LEVEL} 이상)`);
    }
    ...
    if (crossedLevel(strat.from, strat.to, MIN_COMBINED_LEVEL_FOR_GUILD_FEATURES)) {
        pushUnique(
            combined,
            `길드 기능 이용 (유저 Lv.${MIN_COMBINED_LEVEL_FOR_GUILD_FEATURES} 이상)`,
        );
    }
    ...
    for (const b of BORDER_POOL) {
        ...
        if (crossedLevel(strat.from, strat.to, need)) {
