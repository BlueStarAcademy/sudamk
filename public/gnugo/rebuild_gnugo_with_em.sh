...
# Use local/gnugo-3.8 from build_gnugo.sh (same as build output)
G="$SELF/local/gnugo-3.8"

if [ ! -d "$G" ]; then
  echo "Run build_gnugo.sh first to create $G"
  exit 1
fi

mkdir -p "$G/dist/reb/utils" "$G/dist/reb/sgf" "$G/dist/reb/engine" "$G/dist/reb/patterns"
pushd "$G/dist"

find reb -name "*.a" -type f -exec rm -f {} \; 2>/dev/null || true
find reb -name "*.o" -type f -exec rm -f {} \; 2>/dev/null || true
...
# Use patched gnugowrapper.c from repo (with playPass)
cp "$SELF/gnugowrapper.c" .

# Export _playPass so app can replay pass moves (EXPORTED_FUNCTIONS keeps only these; add _playPass)
...
mv a.out.js "$SELF/dist/gnugo.js"
echo "Done: $SELF/dist/gnugo.js"
